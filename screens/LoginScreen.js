...
export default class LoginScreen extends Component { //Exporta a classe LoginScreen
    constructor(props) {
        super(props);
        this.state = {}; //Estado initial esta vazio
    }

    componentDidMount() {
        //Nao sei porque nao tem nada sobre o componente mas geralmente tinha algo sobre tema

    }

    isUserEqual = (googleUser, firebaseUser) => {
        //Checa se o usuario do Google ja esta logado no firebase
        if (firebaseUser) {
    ...
    signInWithGoogleAsync = async () => {
        ...
        try {
            const result = await Google.logInAsync({
                //Configuracao de login do Google
                behaviour: "web",
                androidClientId:
            ...
            if (result.type === "success") {
                //Quando o sign-in da certo, prossiga para o sign-in do firebase
                this.onSignIn(result);
                return result.accessToken;
            } else {
                return { cancelled: true }; //Login cancelado
            }
        } catch (e) {
            //Mensagem de erro
